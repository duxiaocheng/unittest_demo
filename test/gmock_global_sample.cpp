...
TEST(SummizerTestGlobal1, CanSumGlobal1WithOnCall)
{
    ...
    const char* cur_case_name = curTest->test_case_name();
    const char* cur_name = curTest->name();
    printf("cur_case_name:%s, cur_name:%s\n", cur_case_name, cur_name);
...
TEST(SummizerTestGlobal1, case1)
{
    do_test();
}
